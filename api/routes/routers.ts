...
import { hashPassword, verifyPassword, generateToken, validatePasswordStrength, validateEmail, getTokenExpiry, isTokenExpired } from "./auth-utils";
import { z } from "zod";
...
import { examAttemptsRouter } from './routers/examAttempts';
import { certificateGalleryRouter } from './routers/certificateGallery';
import { certificateRenewalRouter } from './routers/certificateRenewal';
import { certificateExpirationRouter } from './routers/certificateExpiration';
import { certificateVerificationRouter } from './routers/certificateVerification';
import { councilTranscriptsRouter } from './routers/councilTranscripts';
...
// ============================================
// COUNCIL APPLICATIONS ROUTER - Analyst Applications
// ============================================
const councilApplicationsRouter = router({
  // Submit application to join the Council
  submit: publicProcedure
    .input(z.object({
      ...
      previousAuditExperience: z.string().optional(),
    ...
    .mutation(async ({ input }) => {
      ...
      const existing = await db
        .select()
        .from(councilApplications)
        .where(eq(councilApplications.email, input.email))
        .limit(1);
      ...
      const [application] = await db.insert(councilApplications).values({
        ...
        relevantCertifications: input.relevantCertifications || null,
        motivation: input.motivation,
        previousAuditExperience: input.previousAuditExperience || null,
        availableHoursPerWeek: input.availableHoursPerWeek || null,
        referralSource: input.referralSource || null,
        status: "pending",
      }).$returningId() as { id: number }[];

      // Send notification to owner about new council application
      try {
        await notifyOwner({
          title: `🏛️ New Council Application: ${input.fullName}`,
          content: `A new application to join the Byzantine Council has been submitted!
...
**Motivation:** ${input.motivation.substring(0, 300)}${input.motivation.length > 300 ? "..." : ""}

Review this application in the CSOAI Dashboard admin panel.`,
        });
      } catch (e) {
        console.warn("Failed to send notification:", e);
      }
  ...
  // Admin: List all council applications
  list: protectedProcedure.query(async ({ ctx }) => {
    if (ctx.user.role !== "admin") {
      throw new Error("Admin access required");
    }
    ...
    return db
      .select()
      .from(councilApplications)
      .orderBy(desc(councilApplications.createdAt));
  }),
  ...
  updateStatus: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db
        .update(councilApplications)
        .set({
          status: input.status,
          reviewNotes: input.reviewNotes || null,
          reviewedBy: ctx.user.id,
        })
        .where(eq(councilApplications.id, input.id));
...
const baseComplianceRouter = router({
  ...
  createAssessment: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const totalScore = assessmentItemsData.reduce((sum: any, item: any) => sum + item.score, 0);
...
This analyst can now review AI safety cases in the Workbench. Your certified analyst pool is growing!`,
  ...
  getExamAnalytics: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const averageScore = attempts.reduce((sum: any, a: any) => sum + (Number(a.percentScore) || 0), 0) / totalAttempts;

      // Calculate average completion time
      const completionTimes = attempts
        .filter((a: any) => a.startedAt && a.completedAt)
        .map((a: any) => {
      ...
      const averageCompletionTime = completionTimes.length > 0
        ? completionTimes.reduce((sum: any, t: any) => sum + t, 0) / completionTimes.length
        : 0;
      ...
      const scoreDistribution = scoreRanges.map((bucket: any) => {
      ...
      const questionStats = allQuestions.map((q: any) => {
        let totalAttempts = 0;
      ...
      const mostMissedQuestions = questionStats
        .filter((q: any) => q.totalAttempts >= 3) // Only include questions with at least 3 attempts
  ...
  getComplianceTrends: regulatorProcedure.query(async () => {
    ...
    const trends = Object.entries(monthlyData).map(([month, data]) => ({
      ...
      avgScore: data.scores.length > 0 
        ? Math.round(data.scores.reduce((a: any, b: any) => a + b, 0) / data.scores.length)
        : 0,
    }));

    return trends.sort((a: any, b: any) => a.month.localeCompare(b.month));
  }),
...
export const appRouter = router({
  ...
  auth: router({
    ...
    signup: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        try {
          ...
          const baseUrl = ctx.req?.headers?.origin || process.env.VITE_FRONTEND_URL;
          await sendVerificationEmail(input.email, input.name, emailVerificationToken, baseUrl);
          console.log('[Signup] Verification email sent to:', input.email);
        } catch (emailError) {
    ...
    verifyEmail: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const userResult = await db.select().from(users).where(eq(users.emailVerificationToken, input.token)).limit(1);
        if (userResult.length === 0) {
        ...
        if (user.emailVerificationExpires && isTokenExpired(new Date(user.emailVerificationExpires))) {
    ...
    resendVerificationEmail: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (user.emailVerified === 1) {
          return { success: true, message: 'Email is already verified. You can log in.' };
        }
        ...
        await db.update(users).set({
          ...
          emailVerificationExpires: newExpiry.toISOString(),
          updatedAt: new Date().toISOString(),
        }).where(eq(users.id, user.id));
        ...
        try {
          const { sendVerificationEmail } = await import('./_core/email');
          const baseUrl = ctx.req?.headers?.origin || process.env.VITE_FRONTEND_URL;
          await sendVerificationEmail(input.email, user.name || 'User', newToken, baseUrl);
    ...
    resetPassword: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        // Check if token is expired
        if (user.passwordResetExpires && isTokenExpired(new Date(user.passwordResetExpires))) {
  ...
  councilApplications: councilApplicationsRouter,
  examAttempts: examAttemptsRouter,
  certificateGallery: certificateGalleryRouter,
  certificateRenewal: certificateRenewalRouter,
  certificateExpiration: certificateExpirationRouter,
  certificateVerification: certificateVerificationRouter,
  councilTranscripts: councilTranscriptsRouter,
