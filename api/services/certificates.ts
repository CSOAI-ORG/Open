        ...
        try {
          ...
          if (userEmail) {
            await sendCertificateEmail({
              ...
              courseName: course.title,
              framework: course.framework || 'AI Governance',
              certificateId,
              completionDate: new Date(enrollment.completedAt || new Date()),
              finalExamScore: (enrollment.score as number) || undefined,
              pdfBuffer,
              verificationUrl: `${process.env.VITE_FRONTEND_URL || 'https://coai.manus.space'}/verify-certificate/${certificateId}`,
            });
            console.log(`[Certificate] Email sent to ${userEmail} for certificate ${certificateId}`);
          }
        } catch (emailError) {
