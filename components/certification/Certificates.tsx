...
export default function Certificates() {
  ...
  const stats = {
    ...
    avgScore: Math.round(mockCertificates.reduce((sum: any, c: any) => sum + c.score, 0) / mockCertificates.length),
  };

  const handleDownloadCertificate = (cert: Certificate): void => {
    toast.success('Certificate downloaded', {
      description: `${cert.title} has been downloaded`,
    });
  };

  const handleShareCertificate = (cert: Certificate): void => {
  ...
  const handleTogglePublic = (cert: Certificate): void => {
  ...
  const daysUntilExpiry = (cert: Certificate): number | null => {
      ...
      <div className="px-6 py-6">
        {filteredCertificates.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCertificates.map((cert: any, idx: number) => (
              <motion.div
                key={cert.id}
