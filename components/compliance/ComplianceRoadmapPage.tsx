...
export const ComplianceRoadmapPage: React.FC = () => {
  ...
  const toggleActionExpanded = (actionId: string): void => {
  ...
  const totalHours = phases.reduce(
    (sum, phase) => sum + phase.actions.reduce((actionSum: any, action: any) => actionSum + action.estimatedHours, 0),
    0
  );
  ...
  const completedHours = phases.reduce(
    (sum, phase) =>
      ...
      phase.actions
        ...
        .reduce((actionSum: any, action: any) => actionSum + action.estimatedHours, 0),
    0
  );
  ...
  return (
    <div className="space-y-6">
            ...
            {phases.map((phase: any) => (
              <div key={phase.phase} className="space-y-2">
                ...
                <div className="text-xs text-muted-foreground">
                  {(phase.startDate instanceof Date ? phase.startDate : new Date(phase.startDate)).toLocaleDateString()} → {(phase.endDate instanceof Date ? phase.endDate : new Date(phase.endDate)).toLocaleDateString()}
                </div>
              </div>
            ))}
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-2xl font-bold">
                      {phase.actions.reduce((sum: any, action: any) => sum + action.estimatedHours, 0)}h
                    </p>
                  </div>
                  <div className="space-y-2">
                          ...
                          <div className="pt-3 border-t space-y-3">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                              ...
                              <div>
                                ...
                                <p className="font-semibold text-sm">{(action.startDate instanceof Date ? action.startDate : new Date(action.startDate)).toLocaleDateString()}</p>
                              </div>
                              <div>
                                <p className="text-xs text-muted-foreground">End Date</p>
                                <p className="font-semibold text-sm">{(action.endDate instanceof Date ? action.endDate : new Date(action.endDate)).toLocaleDateString()}</p>
                              </div>
                            </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {['Security Officer', 'Compliance Officer', 'Product Manager', 'Operations Manager'].map((owner: any) => {
              const ownerActions = phases.flatMap((p) => p.actions).filter((a: any) => a.owner === owner);
              const ownerHours = ownerActions.reduce((sum: any, a: any) => sum + a.estimatedHours, 0);
