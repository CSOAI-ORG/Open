...
export function ComplianceRulesEngine() {
  ...
  const jurisdictions = useMemo(() => {
    return ['all', ...new Set(mockRules.map((r: any) => r.jurisdiction))];
  }, []);

  const frameworks = useMemo(() => {
    return ['all', ...new Set(mockRules.map((r: any) => r.framework))];
  }, []);
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold text-red-600">
              {mockRules.filter((r: any) => r.severity === 'critical').length}
            </p>
          </CardContent>
        </Card>
