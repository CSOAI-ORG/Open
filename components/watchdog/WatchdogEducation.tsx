        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <ul className="space-y-1">
              {module.learningOutcomes.map((outcome: any, idx: number) => (
...
export function WatchdogEducation() {
  ...
  const categories = ['All', ...Array.from(new Set(watchdogModules.map((m: any) => m.category)))];
  const filteredModules = selectedCategory
    ? watchdogModules.filter((m: any) => m.category === selectedCategory || selectedCategory === 'All')
    : watchdogModules;

  const handleStartModule = (moduleId: number): void => {
