...
export default function WatchdogIncidentsPanel({ onViewIncident }: WatchdogIncidentsPanelProps) {
  ...
  const [selectedIncident, setSelectedIncident] = useState<{ id: number; title: string } | null>(null);
  const isAdminOrAnalyst = user?.role === 'admin' || user?.role === 'watchdog_analyst';
              ...
              <CardContent className="pt-4">
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isAdminOrAnalyst && incident.status !== 'resolved' && incident.status !== 'dismissed' && (
                    ...
                    {/* Resolution Details */}
                    {(incident.status === 'resolved' || incident.status === 'dismissed') && incident.resolutionNotes && (
                      <div className="mt-3 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                        <div className="flex items-center gap-2 mb-2">
                          <CheckCircle className="h-4 w-4 text-green-600" />
                          <span className="text-sm font-medium text-green-800 dark:text-green-200">
                            {incident.status === 'resolved' ? 'Resolution Details' : 'Dismissal Reason'}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-4 mt-2 text-xs text-green-600 dark:text-green-400">
                          ...
                          {incident.resolutionDate && (
                            <span>Date: {format(new Date(incident.resolutionDate), 'MMM d, yyyy')}</span>
                          )}
                        </div>
                      </div>
                    )}
