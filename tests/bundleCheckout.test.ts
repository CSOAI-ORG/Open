...
describe("Bundle Checkout System", () => {
  ...
  describe("Stripe Integration", () => {
    it("should have Stripe price IDs for bundles", async () => {
      ...
      // Skip if no active bundles in test database
      if (bundles.length === 0) {
        console.log("⚠️ No active bundles found, skipping test");
        return;
      }
      
      for (const bundle of bundles) {
        // stripePriceId may be null for test bundles
        if (bundle.stripePriceId) {
          expect(typeof bundle.stripePriceId).toBe('string');
          expect(bundle.stripePriceId).toMatch(/^price_/);
        }
      }
      
      console.log(`✅ Checked ${bundles.length} active bundles for Stripe price IDs`);
    });
  });

  describe("Coupon Validation", () => {
    it("should validate coupon code format", () => {
      const isValidCouponFormat = (code: string): boolean => {
