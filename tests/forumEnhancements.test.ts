...
describe('Forum Enhancements', () => {
  ...
  describe('Instructor Moderation', () => {
    it('should allow instructor to pin thread', async () => {
      ...
      expect(Boolean(thread.isPinned)).toBe(true);
    });
    ...
    it('should allow instructor to lock thread', async () => {
      ...
      expect(Boolean(thread.isLocked)).toBe(true);
    });
    ...
    it('should allow instructor to mark post as instructor response', async () => {
      ...
      expect(Boolean(post.isInstructorPost)).toBe(true);
    });
  ...
  describe('Integration Tests', () => {
    it('should handle complete workflow: create thread, reply, mark solution, moderate', async () => {
      ...
      const finalThreadResult = await db!
        .select()
        .from(forumThreads)
        .where(eq(forumThreads.id, newThread.threadId))
        .limit(1);
      const finalThread = finalThreadResult?.[0];

      if (finalThread) {
        expect(Boolean(finalThread.isPinned)).toBe(true);
        expect(Boolean(finalThread.isLocked)).toBe(true);
      } else {
        expect(finalThreadResult.length).toBeGreaterThan(0);
      }
