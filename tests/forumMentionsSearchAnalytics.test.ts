...
describe('Forum Mentions, Search, and Analytics', () => {
  ...
  beforeEach(async () => {
    ...
    if (existingCourses.length > 0) {
      ...
      testCourseId = courseResult[0]?.id || 0;
    }
    ...
    testThreadId = threadResult[0]?.id || 0;
  });
  ...
  describe('Analytics Functionality', () => {
    beforeEach(async () => {
      ...
      await db.insert(forumPosts).values({
        threadId: solvedThreadResult[0]?.id || 0,
        userId: testUserId2,
  ...
  describe('Sorting and Filtering', () => {
    ...
    it('should sort threads by recent activity', async () => {
      ...
      // Filter to only 'Old Thread' and 'New Thread' for this test
      const testThreads = threads.filter((t: any) => 
        t.title === 'Old Thread' || t.title === 'New Thread'
      );

      const sorted = testThreads.sort((a: any, b: any) => 
    ...
    it('should sort threads by popularity (view count)', async () => {
      ...
      const sorted = threads.sort((a: any, b: any) => b.viewCount - a.viewCount);
