...
describe('Notification Filter Service', () => {
  ...
  it('should return saved filter presets', () => {
    ...
    expect(presets.some(p => p.id === 'last_7_days_high')).toBe(true);
    expect(presets.some(p => p.id === 'unread')).toBe(true);
    expect(presets.some(p => p.id === 'compliance_alerts')).toBe(true);
  });
  ...
  it('should have critical only filter', () => {
    ...
    if (criticalPreset?.filters.priority) {
      expect(Array.isArray(criticalPreset.filters.priority) ? criticalPreset.filters.priority.includes('urgent') : criticalPreset.filters.priority === 'urgent').toBe(true);
    }
  });
});
